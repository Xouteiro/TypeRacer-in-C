...
int read_KBC_output(uint8_t port, uint8_t *output, uint8_t mouse) {
    ...
    while (attemps) {
        ...
        if ((status & BIT(0)) != 0) {                       // o output buffer está cheio, posso ler
            ...
            if (mouse && !(status & BIT(5))) {              // está à espera do output do rato mas o output não é do rato
                printf("Error: Mouse output not found\n");  
                return 1;
            } 
            if (!mouse && (status & BIT(5))) {                 // está à espera do output do teclado mas o output não é do teclado
