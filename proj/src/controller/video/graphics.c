...
int (set_frame_buffer)(uint16_t mode, uint8_t** frame_buffer) {
  ...
  memset(&mode_info, 0, sizeof(mode_info));             // obter informação 
  if(vbe_get_mode_info(mode, &mode_info)) return 1;     //sobre o @mode

 
  unsigned int frame_size = (mode_info.XResolution * mode_info.YResolution * mode_info.BitsPerPixel) / 8;   // cálculo do tamanho do frame buffer, em bytes
